#pragma once

extern volatile float anc_mu;
extern volatile float anc_eps;
extern volatile float anc_leak;
extern volatile float anc_cancel_gain;
extern volatile float anc_update_sign;
extern volatile int   anc_lag;
extern volatile bool  anc_adapt;
extern volatile bool  anc_off;
extern volatile float anc_ref_threshold;
extern volatile float anc_mavg_weight;
